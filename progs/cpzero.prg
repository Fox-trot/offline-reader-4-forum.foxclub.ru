...
IF USED()
	LOCAL fname, aname, lexlc
	STORE DBF() TO fname
	STORE ALIAS() TO aname
	STORE ISEXCLUSIVE() TO lexlc
	USE
	outbyte = m.cpbyte
	found_one = .F.
	FOR i = 1 TO 22
		IF m.cpbyte = cpnums[m.i,1]
			outbyte = cpnums[m.i,2]
			found_one = .T.
			EXIT
		ENDIF
	ENDFOR
	...
	IF FILE(m.fname)
		fp_in = FOPEN(m.fname,2)
		IF m.fp_in > 0
			* First check that we have a FoxPro table...
			buf=FREAD(m.fp_in,32)	&&c_buf_size
			IF (SUBSTR(m.buf,1,1) = CHR(139) OR SUBSTR(m.buf,1,1) = CHR(203) OR SUBSTR(m.buf,31,1) != CHR(0) OR SUBSTR(m.buf,32,1) != CHR(0))
				=fclose(m.fp_in)
			ELSE
		...
		ENDIF
		IF lexlc
			USE (m.fname) ALIAS (m.aname) EXCLUSIVE
		ELSE
			USE (m.fname) ALIAS (m.aname)
		ENDIF
	ENDIF
ENDIF
